...
# Установка шрифтов
pdf = FPDF('L')  # Добавьте 'L' для альбомной ориентации
...
# Пройтись по каждому ученику и собрать данные
for index, pupil in df_pupils.iterrows():
    ...
    pdf.cell(0, 0, "", border="B", ln=True) # Добавить горизонтальную линию между строками
    ...
    # Получение предметов и оценок для ученика
    query_lessons = f'''
    ...
    # Группировка предметов и расчет средней оценки

    # У меня в программе создаётся таблица по школьным предметам. Добавь проверку, что пока предмет такой же, что и на прошлом цикле, то цвет печати красный.
    ...
    # Расчет количества оценок по всем предметам
