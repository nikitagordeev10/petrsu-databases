...
class Task4:
    ...
    def setup_ui(self):
        ...
        title_label = Label(self.master, text="Агрегацию и вывод результатов", font=title_font_style)
        ...
        # Агрегация
        Label(self.master, text="Агрегация:", font=font_style).grid(row=6, column=0)
        self.aggregation_var = StringVar()
        self.aggregation_entry = ttk.Combobox(self.master, textvariable=self.aggregation_var, values=['Сумма', 'Среднее'], font=font_style)
        self.aggregation_entry.grid(row=6, column=1)

        # Кнопка агрегации
        self.aggregate_button = Button(self.master, text="Выполнить агрегацию", command=self.perform_aggregation, font=font_style)
        self.aggregate_button.grid(row=7, column=0, columnspan=2, pady=10)

        # Текущая коллекция
        Label(self.master, text="Текущая коллекция:", font=font_style).grid(row=8, column=0)
        self.current_collection_label2 = Label(self.master, text="Футбольные команды", font=font_style)
        self.current_collection_label2.grid(row=8, column=1)
        ...
        self.documents_text.grid(row=9, column=0, columnspan=2, padx=40, pady=10)
    ...
    def perform_aggregation(self):
        # Выполнение агрегации по выбранным параметрам
        key = self.key_var.get()
        aggregation_function = self.aggregation_var.get()

        if aggregation_function == 'Сумма':
            aggregation_query = {'$sum': f"${key}"}
        elif aggregation_function == 'Среднее':
            aggregation_query = {'$avg': f"${key}"}
        else:
            print("Некорректная агрегация")
            return

        aggregation_result = list(self.current_collection.aggregate([
            {'$group': {'_id': None, 'result': aggregation_query}}
        ]))

        if aggregation_result:
            result_value = aggregation_result[0]['result']
            rounded_result = round(result_value, 1)
            self.documents_text.config(state=NORMAL)
            self.documents_text.delete(1.0, END)
            self.documents_text.insert(END, f"{aggregation_function} {key}: {rounded_result}")
            self.documents_text.config(state="disabled")
